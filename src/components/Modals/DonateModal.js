...
export default function DonateModal({ requestToDonate, chain, onClose }) {
  ...
  async function submit(e) {
    ...
    writeContract({
      ...
      abi: DonationAbi,
      functionName: "donate",
      args: [BigInt(requestToDonate.id)],
    ...
    <div className="fixed top-0 w-full h-full flex justify-center items-center bg-black/40">
      <div className="bg-white justify-center items-center flex basis-11/12 md:basis-3/4 lg:basis-1/2 p-1 rounded-2xl">
        <div className="w-full">
          ...
          <div className="p-4">
            <h1 className="mx-3 font-bold my-3 border-b">Donate</h1>

            <form className="p-3" onSubmit={submit}>
              <label>Donation ({chain.nativeCurrency.symbol})</label>
              <input
                name="donation"
              ...
              >
                {isPending || isConfirming ? "Donating..." : "Donate"}
              </button>
              {hash && (
